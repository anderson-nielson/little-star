...
export function telaAreia(): Tela {
  ...
  const roda = (modo: string, x: number, miolo: string) =>
    modo === 'rastelo'
      ? `<g data-modo="${modo}" transform="translate(${x} ${Y_FERRAMENTAS})"><circle r="36" fill="transparent"/><circle class="fundo" r="30" fill="${COR.papel}" fill-opacity="0.6" stroke="${COR.ouro}" stroke-width="1.5" stroke-dasharray="4 5"/><g transform="scale(0.85)">${miolo}</g></g>`
      : `<g data-modo="${modo}" transform="translate(${x} ${Y_FERRAMENTAS})"><circle class="fundo" r="36" fill="${COR.papel}" stroke="${COR.ouro}" stroke-width="1.5"/>${miolo}</g>`;
