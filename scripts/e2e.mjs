...
await espera(600);
await conta('opções: buscar versão', '#buscar-versao');
await conta('opções: reiniciar', '#reiniciar');
await page.locator('#buscar-versao').click();
await espera(800);
await shot('24-opcoes');
await page.getByRole('button', { name: 'Voltar ao cantinho', exact: true }).click();
await espera(600);
await conta('pais de volta: vozes', '.voz', 40);
